...
var cp = {
    ...
    requestReport: function(tab, pageSource) {
        ...
        var existingReport = _.findWhere(cp.pageReports, {websiteId: websiteId, path: tabUrl.pathname});
        var cacheReports = options.getLocalStore("cacheReports", 1, "boolean");

        if (cacheReports && existingReport && existingReport.data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.completed) {
                            ...
                            console.log(maxCount, 'max count exceeded');
            ...
            function requestPagechecker(json) {
                var report = _.findWhere(cp.pageReports, {websiteId: websiteId, path: tabUrl.pathname});
